...
\begin{center}
    ...
    \vspace{0.5cm}
    {\large Una Introducción Rigurosa a la Derivada de Medidas}
...
\begin{observacion}
    En todo lo que sigue asumimos familiaridad con los conceptos básicos de teoría de la medida: conjuntos medibles, funciones medibles, integrales de Lebesgue, y convergencia en casi todo punto.
\end{observacion}

\subsection{Cargas y medidas con signo}

\begin{definicion}[Carga]
    Si \( \X \) es una \( \sigma \)-álgebra sobre un conjunto \( X \), entonces una función \( \nu \) a valores reales definida sobre \( \X \)
...
\subsection{Teorema de Descomposición de Hahn}
...
\begin{proof}
    ...
    Luego, \(E\) debe contener algún conjunto con medida negativa. Sea \(n_1\) el primer número natural tal que \(E\) contiene un conjunto medible \(E_1\) que verifica \(\nu(E_1) \leq -1/n_1\). Entonces:
    \[
        \nu(E \setminus E_1) = \nu(E) - \nu(E_1) > \nu(E) > 0
    \]
...
\subsection{Teorema de Descomposición de Jordan}
